...
// Rotas de demonstração para cada nível de log
app.post('/demo/warn', async (req, res) => {
  await registrarLog('WARN', 'Tentativa de acessar recurso sem permissao adequada');
  res.json({ message: 'Log WARN registrado' });
});

app.post('/demo/error', async (req, res) => {
  await registrarLog('ERROR', 'Falha critica ao conectar com banco de dados');
  res.json({ message: 'Log ERROR registrado' });
});

app.post('/demo/debug', async (req, res) => {
  await registrarLog('DEBUG', `Estado atual: ${tarefas.length} tarefa(s) em memoria | uptime: ${Math.round(process.uptime())}s`);
  res.json({ message: 'Log DEBUG registrado' });
});
